...
-define(HELP_RESP_TYPES, [{<<"Tasks">>, fun kz_json:is_json_object/1}
                         ,{<<"Tasks-For">>, fun is_binary/1}
                         ,{<<"Tasks-Module">>, fun is_binary/1}
                         ]).

-define(TASKS_AMQP_KEY(SubKey), <<"tasks.", SubKey/binary>>).
...
bind_q(Q, _Props) ->
    amqp_util:bind_q_to_monitor(Q, ?TASKS_AMQP_KEY(<<"*">>)).

-spec unbind_q(ne_binary(), kz_proplist()) -> 'ok'.
unbind_q(Q, _Props) ->
    amqp_util:unbind_q_from_monitor(Q, ?TASKS_AMQP_KEY(<<"*">>)).
...
publish_help_req(Req, ContentType) ->
    ...
    amqp_util:monitor_publish(Payload, ContentType, ?TASKS_AMQP_KEY(<<"help">>)).
